...
def main():
    ...
    # Run model forward pass to get intermediate representation
    _, rep = model(images)

    # Shift to correct shape
    shape = rep.shape
    rep = rep.view(shape[0], shape[1], -1)
    rep = rep.detach().cpu().numpy()

    # Plot 1 example with TSNE
    rep_embed = TSNE(n_components=2).fit_transform(rep[0, :, :])
    plt.scatter(rep_embed[:, 0], rep_embed[:, 1])
